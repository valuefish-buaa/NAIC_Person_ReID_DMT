...
def do_train(cfg,
    ...
    epochs = start_epoch + cfg.SOLVER.MAX_EPOCHS
    ...
    for epoch in range(start_epoch + 1, epochs + 1):
        ...
        if best_acc < acc_meter.avg:
            best_acc = acc_meter.avg
            if epoch % checkpoint_period == 0:
                torch.save(model.state_dict(), os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + '_{}_acc{:.4f}.pth'.format(epoch, acc_meter.avg)))
...
def do_inference(cfg,
    ...
    for index, loader in enumerate(val_loader):
        if index == 0:
            ...
            reranking_parameter = [20, 6, 0.6]  # [30, 8, 0.6]
        ...
        for n_iter, (img, pid, camid, imgpath) in enumerate(loader):
            with torch.no_grad():
                ...
                if cfg.TEST.FLIP_FEATS == 'on':
                    feat = torch.FloatTensor(img.size(0), model.in_planes).zero_().cuda()
                    for i in range(2):
                        if i == 1:
...
def do_inference_all(cfg,
                     model,
                     val_loader_query,
                     val_loader_gallery,
                     num_query, is_save=True):
    device = "cuda"
    logger = logging.getLogger("reid_baseline.test")
    logger.info("Enter inferencing")
    ...
    DISTMAT_PATH = os.path.join(cfg.OUTPUT_DIR, "distmat_all.npy")
    QUERY_PATH = os.path.join(cfg.OUTPUT_DIR, "query_path_all.npy")
    GALLERY_PATH = os.path.join(cfg.OUTPUT_DIR, "gallery_path_all.npy")

    reranking_parameter = [30, 4, 0.8]  # [30, 4, 0.8]  # [20, 6, 0.6]
    evaluator = R1_mAP(num_query, max_rank=200, feat_norm=cfg.TEST.FEAT_NORM,
                       reranking=cfg.TEST.RE_RANKING)
    evaluator.reset()

    model.eval()
    val_loader = [val_loader_query, val_loader_gallery]
    for index, loader in tqdm(enumerate(val_loader)):
        for n_iter, (img, pid, camid, imgpath) in enumerate(tqdm(loader)):
            with torch.no_grad():
                img = img.to(device)
                if cfg.TEST.FLIP_FEATS == 'on':
                    feat = torch.FloatTensor(img.size(0), model.in_planes).zero_().cuda()
    ...
    data_all, distmat, img_name_q, img_name_g = evaluator.compute(reranking_parameter)
    if is_save:
        np.save(DISTMAT_PATH, distmat)
        np.save(QUERY_PATH, img_name_q)
        np.save(GALLERY_PATH, img_name_g)
        ...
        nowTime = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        with open(os.path.join(cfg.OUTPUT_DIR, 'all_result_{}.json'.format(nowTime)), 'w', encoding='utf-8') as fp:
            json.dump(data_all, fp)
    else:
        with open('/raid/chenby/person_rID/2019/REID2019_A/result.json', 'w', encoding='utf-8') as fp:
            json.dump(data_all, fp)
